...
public:
  ...
  /**
   * @brief removes the min element from the heap
   * @return int the min element. return -1 if heap is empty
   */
  int removeMin();
  ...
  /**
   * @brief Runs a heap sort by creating a heap out of the values in the array,
   * and then extracting those values one-by-one from the heap back into the
